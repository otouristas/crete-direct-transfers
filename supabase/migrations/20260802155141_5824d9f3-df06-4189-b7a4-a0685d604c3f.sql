alter table public.bookings
  ...
  add column if not exists cancellation_reason text
    check (
      cancellation_reason is null or cancellation_reason in (
        'customer_plans_changed','customer_booked_wrong','flight_cancelled_airline',
        'traveler_no_show','driver_no_show','driver_late','wrong_vehicle_or_service',
        'safety_concern','missed_each_other','other'
      )
    ),
  ...
  add column if not exists refund_status text not null default 'none'
    check (refund_status in ('none','pending_review','approved','rejected','paid','credit_issued','n_a')),
...
create table if not exists public.booking_incidents (
  ...
  type text not null check (type in (
    'driver_no_show','driver_late','wrong_vehicle','safety','missed_each_other','unable_to_complete','other'
  )),
  ...
  resolution text
    check (resolution is null or resolution in ('full_refund','partial_refund','credit','rebook','no_action')),
...
create table if not exists public.quotes (
  ...
  distance_km numeric,
  hours integer,
  service text not null default 'transfer' check (service in ('transfer', 'hourly')),
  market text not null default 'greece',
  bookable_mode text not null default 'instant' check (bookable_mode in ('instant', 'quote')),
...
create or replace function public.booking_wait_minutes(
  p_pickup_address text, p_route_slug text, p_flight_number text
)
returns integer
...
create policy "customers read own incidents" on public.booking_incidents
  ...
  using (
    exists (select 1 from public.bookings b where b.id = booking_id and public.owns_booking(b))
    or exists (select 1 from public.bookings b where b.id = booking_id and b.driver_id = auth.uid())
    or public.is_admin()
  );
...
create or replace function public.request_cancellation(
  p_booking_id uuid, p_reason text, p_note text default null, p_prefer_credit boolean default true
)
...
begin
  ...
  if p_reason is null or p_reason not in (
    'customer_plans_changed','customer_booked_wrong','flight_cancelled_airline','other'
  ) then
  ...
  if v_booking.status not in ('pending', 'claimed') then raise exception 'cancel_not_allowed'; end if;
  if v_booking.pickup_at <= now() then raise exception 'use_incident_flow'; end if;
...
revoke execute on function public.request_cancellation(uuid, text, text, boolean) from public, anon;
...
create or replace function public.open_incident(
  p_booking_id uuid, p_type text, p_note text default null,
  p_claimed_wait_until timestamptz default null, p_evidence_urls text[] default '{}'
)
...
begin
  ...
  insert into public.booking_incidents (
    booking_id, opened_by, opened_by_user_id, type, note, claimed_wait_until, evidence_urls, status
  ) values (
    p_booking_id, v_opened_by, auth.uid(), p_type,
    nullif(trim(coalesce(p_note, '')), ''), p_claimed_wait_until, coalesce(p_evidence_urls, '{}'), 'open'
...
revoke execute on function public.open_incident(uuid, text, text, timestamptz, text[]) from public, anon;
...
create or replace function public.resolve_incident(
  p_incident_id uuid, p_resolution text, p_notes text default null, p_reject boolean default false
)
returns public.booking_incidents
language plpgsql security definer set search_path = public as $$
declare
  v_incident public.booking_incidents;
  v_booking public.bookings;
  v_goodwill integer := 2500;
begin
  if not public.is_admin() then raise exception 'forbidden'; end if;

  if p_reject then
    update public.booking_incidents set
      status = 'rejected', resolution = 'no_action',
  ...
  update public.booking_incidents set
    status = 'resolved', resolution = p_resolution,
    resolution_notes = nullif(trim(coalesce(p_notes, '')), ''),
    resolved_by = auth.uid(), resolved_at = now(), updated_at = now()
  where id = p_incident_id
  returning * into v_incident;
  ...
  if p_resolution = 'full_refund' then
    ...
    update public.bookings set
      refund_status = 'n_a', goodwill_credit_cents = v_goodwill, updated_at = now()
    where id = v_booking.id;
  else
    update public.bookings set
      refund_status = case when refund_status = 'pending_review' then 'rejected' else refund_status end,
...
revoke execute on function public.resolve_incident(uuid, text, text, boolean) from public, anon;
grant execute on function public.resolve_incident(uuid, text, text, boolean) to authenticated;

create or replace function public.update_job_status(p_booking_id uuid, p_status text)
returns public.bookings
language plpgsql security definer set search_path = public as $$
declare
  v_booking public.bookings;
...
begin
  if auth.uid() is null then raise exception 'not_authenticated'; end if;
  if p_status not in ('en_route', 'completed', 'no_show') then raise exception 'invalid_status'; end if;

  select * into v_booking from public.bookings where id = p_booking_id for update;
  if not found then raise exception 'booking_not_found'; end if;

  if v_booking.driver_id is distinct from auth.uid() then raise exception 'forbidden'; end if;
  if p_status = 'en_route' and v_booking.status <> 'claimed' then raise exception 'update_refused'; end if;
  ...
  if p_status = 'no_show' then
    ...
    update public.bookings set
      status = 'no_show', cancelled_by = 'driver', cancellation_reason = 'traveler_no_show',
      cancelled_at = now(), refund_percent = 0,
      refund_status = case when payment_status = 'unpaid' then 'n_a' else 'none' end,
      refund_amount_cents = 0, updated_at = now()
    where id = p_booking_id
    returning * into v_booking;
  else
...
revoke execute on function public.update_job_status(uuid, text) from public, anon;
...
create or replace function public.create_quote_record(
  p_route_slug text, p_vehicle_class text, p_trip_type text, p_extras jsonb,
  p_pickup_at timestamptz, p_return_at timestamptz, p_distance_km numeric, p_hours integer,
  p_service text, p_market text, p_bookable_mode text, p_price_cents integer,
  p_breakdown jsonb, p_inputs_hash text, p_ttl_minutes integer default 30
)
...
begin
  ...
  insert into public.quotes (
    ...
    p_inputs_hash, p_route_slug, p_vehicle_class, coalesce(p_trip_type, 'oneway'),
    coalesce(p_extras, '{}'::jsonb), p_pickup_at, p_return_at, p_distance_km, p_hours,
    coalesce(p_service, 'transfer'), coalesce(p_market, 'greece'),
    coalesce(p_bookable_mode, 'instant'), p_price_cents, coalesce(p_breakdown, '[]'::jsonb),
...
grant insert, update, delete on public.partner_referrals to authenticated;
grant all on public.partner_referrals to service_role;
grant select, insert, update, delete on public.booking_referrals to authenticated;
grant all on public.booking_referrals to service_role;
    ...
    if v_driver is not null then
      update public.driver_profiles
        set dns_strikes = dns_strikes + 1,
            approval_status = case when dns_strikes + 1 >= 3 then 'suspended' else approval_status end,
            updated_at = now()
      where id = v_driver
